...
import {
  getPenjualanOrder,
  getPenjualanOrderCount,
  reset,
} from "../../features/laporanPenjualanOrderSlice";

const PenjualanOrder = ({ jenis, sumColumn = [] }) => {
  ...
  const { data, status, message, dataCount } = useSelector(
    (state) => state.laporanPenjualanOrder
  );
  ...
  const handleReset = () => {
    ...
    dispatch(getPenjualanOrder(initialState));
    dispatch(getPenjualanOrderCount({ ...initialState, count_stats: 1 }));
    setPage(0);
  };
  ...
  const handleFilter = (e) => {
    ...
    dispatch(getPenjualanOrder({ ...formData, limit: 0 }));
    dispatch(getPenjualanOrderCount({ ...formData, count_stats: 1, limit: 0 }));
    setPage(0);
  };
  ...
  const handlePaginate = (e) => {
    ...
    dispatch(getPenjualanOrder({ ...formData, limit: newOffset }));
    setPage(e.selected);
  };
